	...
	<?php if ( is_active_sidebar( 'sidebar-2' ) ):
  			  ...
  			  if ( current_user_can( 'edit_theme_options' ) && ! has_nav_menu( 'sidebar' ) ) { ?>
  				<section class="widget-container widget-placeholder">
  					<h3 class="widget-title"><?php _e( 'Left Sidebar', 'anima' ); ?></h3>
  					<p>
  						<?php
  								printf( __( 'You currently have no widgets set in this sidebar. You can add widgets via the <a href="%s">Dashboard</a>.', 'anima' ), esc_url( admin_url() . "widgets.php" ) ); echo "<br/>";
  								printf( __( 'To hide this sidebar, switch to a different Layout via the <a href="%s">Theme Customizations</a>.', 'anima' ), esc_url( admin_url() . "customize.php" ) );
  						?>
  					</p>
  				</section>
  			<?php }
