	...
	if ( $anima_meta_author ) {
		echo sprintf(
			'<span class="author vcard"' . cryout_schema_microdata( 'author', 0 ) . '>
				...
				<a style="cursor: none;" class="url fn n" rel="author" href="javascript:void(0)" title="%2$s"' . cryout_schema_microdata( 'author-url', 0 ) . '>
		...
		<div class="post-thumbnail-container"  <?php cryout_schema_microdata( 'image' ); ?>>
			...
			<a class="responsive-featured-image" href="<?php echo esc_url( get_permalink( $post->ID ) ) ?>" title="<?php echo esc_attr( get_post_field( 'post_title', $post->ID ) ) ?>">
				<picture>
	 				...
	 				<?php if ( cryout_on_landingpage() ) { ?><source sizes="<?php echo cryout_gen_featured_sizes( $featured_width, $options['anima_magazinelayout'], $options['anima_landingpage'] ) ?>" srcset="<?php echo cryout_get_picture_src( $fimage_id, 'anima-featured-lp' ); ?> <?php printf( '%sw', $featured_width ) ?>">
					<?php } ?>
