...
abstract class Util
{
    ...
    {
        $types = 'v1' === $apiMode ? \ProfilePressVendor\Stripe\Util\ObjectTypes::mapping : \ProfilePressVendor\Stripe\Util\ObjectTypes::v2Mapping;
        if (self::isList($resp)) {
            $mapped = [];
            foreach ($resp as $i) {
        ...
        if (\is_array($resp)) {
            if (isset($resp['object']) && \is_string($resp['object']) && isset($types[$resp['object']])) {
                ...
                if ('v2' === $apiMode && 'v2.core.event' === $resp['object']) {
                    $eventTypes = \ProfilePressVendor\Stripe\Util\EventTypes::thinEventMapping;
                ...
                $class = \ProfilePressVendor\Stripe\V2\Collection::class;
            } else {
                $class = \ProfilePressVendor\Stripe\StripeObject::class;
            }
        ...
        foreach ($properties as $key => $property) {
            if (\array_key_exists($property->getName(), $json)) {
                if ('related_object' === $property->getName()) {
                    $related_object = new \ProfilePressVendor\Stripe\RelatedObject();
                    ...
                    $reason = new \ProfilePressVendor\Stripe\Reason();
    ...
    {
        if ($h instanceof \ProfilePressVendor\Stripe\ApiResource) {
            return $h->id;
        }
        if (static::isList($h)) {
            $results = [];
