...
trait Request
{
    ...
    protected static function _validateParams($params = null)
    {
        if ($params && !\is_array($params)) {
            ...
            throw new \ProfilePressVendor\Stripe\Exception\InvalidArgumentException($message);
        }
    }
    /**
    ...
    protected static function _requestPage($url, $resultClass, $params = null, $options = null, $usage = [])
    {
        ...
        if (!$obj instanceof $resultClass) {
            throw new \ProfilePressVendor\Stripe\Exception\UnexpectedValueException('Expected type ' . $resultClass . ', got "' . \get_class($obj) . '" instead.');
        }
    ...
    {
        $opts = \ProfilePressVendor\Stripe\Util\RequestOptions::parse($options);
        $baseUrl = isset($opts->apiBase) ? $opts->apiBase : static::baseUrl();
        $requestor = new \ProfilePressVendor\Stripe\ApiRequestor($opts->apiKey, $baseUrl);
