...
namespace ProfilePressVendor\League\Csv\Polyfill;

use Generator;
use ProfilePressVendor\League\Csv\Stream;
use SplFileObject;
use TypeError;
...
final class EmptyEscapeParser
{
    ...
    public static function parse($document) : Generator
    {
        ...
        self::$trim_mask = str_replace([self::$delimiter, self::$enclosure], '', " \t\x00\v");
        self::$document->setFlags(0);
        self::$document->rewind();
        while (self::$document->valid()) {
            $record = self::extractRecord();
            if ([null] === $record || !in_array(null, $record, \true)) {
                (yield $record);
            }
        }
    }
    ...
    private static function filterDocument(object $document)
    {
        ...
        throw new TypeError(sprintf('%s::parse expects parameter 1 to be a %s or a SplFileObject object, %s given', self::class, Stream::class, get_class($document)));
    }
    /**
    ...
    {
        if (in_array(self::$line, self::FIELD_BREAKS, \true)) {
            self::$line = \false;
            return null;
        }
