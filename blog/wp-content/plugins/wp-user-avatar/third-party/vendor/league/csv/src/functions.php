...
function delimiter_detect(Reader $csv, array $delimiters, int $limit = 1) : array
{
    ...
    $delimiter_stats = static function (array $stats, string $delimiter) use($csv, $stmt, $record_filter) : array {
        ...
        $found_records = array_filter(iterator_to_array($stmt->process($csv)->getRecords(), \false), $record_filter);
    ...
    $stats = array_reduce(array_unique(array_filter($delimiters, $delimiter_filter)), $delimiter_stats, array_fill_keys($delimiters, 0));
